...
with colon-delimited fields of the form:
  ...
  username[:selector]:tokenid:key[:pin]

The C<selector> and C<pin> fields are optional.
...
The C<selector> field is a single character used to distinguish between
multiple tokens held by the same user.  The user must enter the selector
character and a colon as the first two characters of the passcode, and
otpd must have access to the plaintext of the OTP, i.e. for CHAP-style
authentications token selectors are unavailable.

=head2 tokenid

Only alphanumeric characters and C<-> (dash) are valid for the C<tokenid>
field.  This field must be at least 2 characters long to distinguish it
from the optional C<selector> field.

If using selectors, the C<tokenid> field must be unique across all tokens.
Normally this would be accomplished by including a serial number.
...
restriction noted earlier does not apply to the PIN itself.  However,
if token selectors are used then colons need to be disallowed or
...
defines the user C<bob>, who holds an C<hotp-d6> token, with key
...
Each of the token ids described below can be suffixed with C<-> (dash)
and then arbitrary alphanumeric text, e.g. a serial number, to meet
the uniqueness requirement for using selectors.
...
using a 128-bit to 256-bit key and varying response lengths.
